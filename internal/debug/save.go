...
func (p *Policy) saveRequest(r *http.Request, dir string) {
	...
	if _, err := fmt.Fprintf(f, "%s %s HTTP/1.1\r\n", r.Method, r.URL.RequestURI()); err != nil {
		log.Printf("[debug] save request: write %s: %v", name, err)
		_ = f.Close()
		return
	}
	if r.Host != "" {
	...
	if r.Body == nil {
		...
		r.Body = &teeBody{body: r.Body, f: f, path: path}
	}
}
...
func (b *teeBody) Close() error {
	...
	if err := b.dump(); err != nil {
		log.Printf("[debug] save request: write body to %s: %v", b.path, err)
	} else {
		log.Printf("[debug] saved request to %s", b.path)
	}
	err := b.body.Close()
